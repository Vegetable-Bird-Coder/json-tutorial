...
static int lept_parse_object(lept_context *c, lept_value *v) {
    ...
    for (i = 0; i < size; i++) {
        lept_member *m =
            (lept_member *)lept_context_pop(c, sizeof(lept_member));
        free(m->k);
        lept_free(&m->v);
    }
    v->type = LEPT_NULL;
...
void lept_copy(lept_value *dst, const lept_value *src) {
    ...
    case LEPT_OBJECT:
        ...
        for (i = 0; i < dst->u.o.size; i++) {
            lept_member *co = dst->u.o.m + i;
            lept_member *o = src->u.o.m + i;
            co->klen = o->klen;
            co->k = (char *)malloc((o->klen + 1) * sizeof(char));
            memcpy(co->k, o->k, co->klen);
            co->k[co->klen] = '\0';
            lept_copy(&co->v, &o->v);
        }
        break;
...
int lept_is_equal(const lept_value *lhs, const lept_value *rhs) {
    ...
    case LEPT_OBJECT:
        ...
        for (i = 0; i < lhs->u.o.size; i++) {
            lept_member *o = lhs->u.o.m + i;
            lept_value *v = lept_find_object_value(rhs, o->k, o->klen);
            if (v == NULL || !lept_is_equal(&o->v, v))
                return 0;
        }
        return 1;
    default:
...
lept_value *lept_insert_array_element(lept_value *v, size_t index) {
    ...
    /* \todo */
    lept_value *last = lept_pushback_array_element(v);
    size_t i = 0;
...
void lept_shrink_object(lept_value *v) {
    ...
    if (v->u.o.capacity > v->u.o.size) {
        ...
        v->u.o.m = (lept_member *)realloc(v->u.o.m, v->u.o.capacity *
                                                        sizeof(lept_member));
    }
}
...
lept_value *lept_set_object_value(lept_value *v, const char *key, size_t klen) {
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    /* \todo */
    lept_value *find = lept_find_object_value(v, key, klen);
    if (find != NULL)
        return find;
...
void lept_remove_object_value(lept_value *v, size_t index) {
    ...
    /* \todo */
    lept_member *remove = v->u.o.m + index;
    free(remove->k);
    lept_free(&remove->v);
    if (index == v->u.o.size - 1)
        free(remove);
    else {
        lept_member *last = v->u.o.m + v->u.o.size - 1;
        remove->k = last->k;
        lept_move(&remove->v, &last->v);
    }
    v->u.o.size--;
}
